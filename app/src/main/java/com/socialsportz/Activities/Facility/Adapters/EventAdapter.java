...
import static com.socialsportz.Constants.Constants.kImageBaseUrl;
 ...
 public class EventAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public EventAdapter(Context context, CopyOnWriteArrayList<Events> mData, onItemClickListener listener) {
     ...
     public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
         if (viewType == 1) {
             View layoutView = LayoutInflater.from(context)
                     .inflate(R.layout.row_event_view, parent, false);
     ...
     public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
         if (position == mData.size()) {
             ...
             myViewHolder.tv_avail_booking.setText(events.getAvailable_slot_count());
    ...
    class myViewHolder extends RecyclerView.ViewHolder {
        ImageView ivBanner;
        TextView field_name,venue,date,participants,booked,status,tv_pstatus,tv_avail_booking;
        private myViewHolder(View itemView) {
			...
			tv_avail_booking=itemView.findViewById(R.id.tv_avail_booking);
        }
    }
