...
import com.socialsportz.Activities.User.Adapters.FacilitySeeAllAdapter;
...
public class FacilitySeeAllActivity extends AppCompatActivity {

	private static final String TAG = NotificationActivity.class.getSimpleName();
	private Context context;
	private RecyclerView rv_myEnquire;
	private LinearLayoutManager mLayoutManager;
	private FacilitySeeAllAdapter userNotificationAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_facility_see_all);
		context = this;
	...
	public void inItView() {
		...
		tv_page_title.setText("Facility");
	...
	private void getSearchListing() {
		...
		map.put(kType, "facility");
		map.put(kUserId,ModelManager.modelManager().getCurrentUser().getUserId());
		Log.e(TAG, map.toString());
		ModelManager.modelManager().userFacilityAllList(map,
			(Constants.Status iStatus, GenericResponse<UserFacility> genericResponse) -> {
				...
				try {
					...
					setRecyclerView(dashBoardData.getFacilities());
	...
	private void setRecyclerView(CopyOnWriteArrayList<UserFacAca> userEnquires) {
		userNotificationAdapter = new FacilitySeeAllAdapter(context, userEnquires);
