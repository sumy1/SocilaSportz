...
public class EventListing extends BaseModel implements Serializable {
    ...
    public EventListing(JSONObject jsonResonse){
        ...
        this.sportIcon=getValue(jsonResonse,kSportImage,String.class);
        ...
        if(jsonResonse.has(kEventBookings)){
            try {
                this.booked=jsonResonse.getInt(kEventBookings);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }

        //this.booked = Integer.valueOf(getValue(jsonResonse,kEventBookings,String.class));
        ...
        if(jsonResonse.has(kSportListing)) {
            try {
                this.facSportList = handleFacSports(getValue(jsonResonse, kSportListing, JSONArray.class));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
        ...
        if(jsonResonse.has(kGalleryList)) {
            try {
                this.facGalleryList = handleFacViewGallery(getValue(jsonResonse, kGalleryList, JSONArray.class));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    private CopyOnWriteArrayList<UserEventViewGallery> handleFacViewGallery(JSONArray jsonArray) throws JSONException {
    ...
    public EventListing(String image){
        this.image = image;
        this.banner = "";
    }
