...
public class UserFacAca extends BaseModel implements Serializable {
    ...
    public UserFacAca(JSONObject jsonResponse){
        ...
        if(jsonResponse.has(kSlotCount)){
			try {
				this.slot_count=jsonResponse.getInt(kSlotCount);
			} catch (JSONException e) {
				e.printStackTrace();
			}
		}

        //this.slot_count=Integer.parseInt(getValue(jsonResponse,kSlotCount,String.class));
    ...
    public UserFacAca(int id, int image) {
        this.facId = id;
        this.image = image;
        this.facBannerImg = "";
    }
