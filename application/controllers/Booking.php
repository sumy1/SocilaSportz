...
## slot addto cart here *******************************
	public function add_to_cart(){
		...
		if(count($slots_batch_ids)>0){
			...
			if(!empty($res)){
				$data['cart_list_arr'] = $this->CommonMdl->getResultByCond('tbl_cart', array('sport_id'=>$this->session->userdata('book_sport_id'), 'fac_id'=>$this->session->userdata('book_fac_id'), 'session_id'=>$this->session->userdata('session_id')),'*');
				//echo "<pre>";
				//print_r($data['cart_list_arr']); die;
	...
	public function booking_now(){
		...
		if(!empty($this->session->userdata('user_id')) && !empty($this->session->userdata('book_fac_id')) && !empty($this->session->userdata('book_sport_id'))){
			...
			if ($cart_list_arr[0]->trial_booking!='yes') {
			...
			if ($this->input->post('coupon_code') == $this->input->post('coupon_code_hidden')) {	
				$coupon = $this->CommonMdl->getRow('tbl_coupon','coupon_code, coupon_flat_amount', array(
					'coupon_code'=>$this->input->post('coupon_code'),
					'cart_min_amount<='=>$total_amount,
					'coupon_start_date<='=>date('Y-m-d'),
					'coupon_end_date>='=>date('Y-m-d')
				));
			//echo $this->db->last_query(); die;
				if ($coupon) {
					$coupon_amount = $coupon->coupon_flat_amount;
			...
			if($insert_booking_id){
				foreach($cart_list_arr as $listVal){ 
					...
					$booking_detail_arr[]=array('booking_date'=>$listVal->book_date,
						'slot_time'=>$slot_time,
				...
				$fac_detail = $this->CommonMdl->getRow('tbl_facility','fac_name, fac_type', array(
					'fac_id'=>$listVal->fac_id));
				$sport_detail = $this->CommonMdl->getRow('tbl_sport_list','sport_name, ', array('sport_id'=>$book_sport_id));
                   $ArrNotification=array(
						  'fac_id'=>$listVal->fac_id,
						  'notification_title'=>'Booking',
						  'notification_message'=>'Your have got 1 booking',
						  'booking_id'=>$booking_order_no,
						  'booking_email'=>$user_details->user_email,
						  'created_on'=>date('Y-m-d H:i:s'),
						  'updated_on'=>date('Y-m-d H:i:s')
					  );
					  $this->CommonMdl->insertRow($ArrNotification,'tbl_notification');
				...
				redirect(base_url('dashboard/user_booking'));
			}
			...
			if($insert_booking_id){
				foreach($cart_list_arr as $listVal){ 
					...
					$slot_book_arr = array(
						...
						'batch_slot_id' => $listVal->batch_slot_id,
						'sport_id' => $book_sport_id,
						'booking_detail_total_amount' => 0,
						'booking_detail_final_amount' => 0,
						'batch_slot_start_time' => $listVal->start_time,
						'batch_slot_end_time' => $listVal->end_time,
						'start_date' => $listVal->book_date,
						'batch_slot_type_id' => @$bs_type_detail->batch_slot_type_id,
	...
	public function event_booking_now(){	
		if(!empty($this->session->userdata('user_id')) && !empty($this->session->userdata('book_event_id'))){
			...
			$coupon = $this->CommonMdl->getRow('tbl_coupon','coupon_code, coupon_flat_amount', array(
				'coupon_code'=>$this->input->post('coupon_code'),
				'cart_min_amount<='=>$event_details->event_price,
				'coupon_start_date<='=>date('Y-m-d'),
				'coupon_end_date>='=>date('Y-m-d')
			));
			//echo $this->db->last_query(); die;
			...
			$book_data_arr = array(
				...
				'final_amount' => $event_details->event_price-$coupon_amount,
				'discount_amount' => $coupon_amount,
				'coupon_code' => $coupon_code,
				'booking_for' => 'event',
				'payment_status' => 'pending',
			...
			if($insert_booking_id){
				...
				$fac_detail = $this->CommonMdl->getRow('tbl_facility','fac_name, fac_type', array(
					'fac_id'=>$event_details->fac_id));
			/*$bookingorder_detail = $this->CommonMdl->getRow('tbl_booking_order','booking_type', array(
			'user_id'=>$user_id));*/
			...
			$data['booking_no']=$booking_order_no;
					//$this->load->view('public/ThanksView',$data);
			redirect(base_url('dashboard/user_booking/event'));
