...
public function user_booking(){
	$data['get_booking_order']=$this->CommonMdl->getResultByCond('tbl_booking_order',array('user_id'=>$this->session->userdata['user_id'],'booking_for'=>'facility'),'booking_order_no,booking_order_id,payment_status,name',array('col_name'=>'booking_order_id','order'=>'desc'));
	 
	foreach($data['get_booking_order'] as $bookingorderkey=>$bookingorderval){
				$data['get_slot_detail']=$this->CommonMdl->getResultByCond('tbl_booking_slot_detail',array('booking_order_id'=>$bookingorderval->booking_order_id),'booking_order_id,fac_id,start_date,batch_slot_start_time,batch_slot_end_time',$order_by='');
			...
			/*foreach($data['get_booking_order'][$bookingorderkey]->get_slot_detail as $booking_slotkey=>$booking_slotval){
			  $data['get_facility']=$this->CommonMdl->getResultByCond('tbl_facility',array('fac_id'=>$booking_slotval->fac_id),'fac_name,fac_id,fac_banner_image',$order_by='');
			 $data['get_booking_order'][$bookingorderkey]->get_slot_detail[$booking_slotkey]->facility_name=$data['get_facility'];
		    }*/
		}
		
		
		$data['get_booking_orders']=$this->CommonMdl->getResultByCond('tbl_booking_order',array('user_id'=>$this->session->userdata['user_id'],'booking_for'=>'event'),'booking_order_no,booking_order_id,name',$order_by='');
...
$html['_html'] = $this->load->view('public/user-dashboard/UserSingleEventView',$data,true);
  return $this->output->set_content_type('application/json')->set_output(json_encode($html));
    ...
    public function user_booking_view_model(){
	  ...
	  foreach($data['order_main_detail']->order_sub_detail as $slotdetailkey=>$slotdetailval){
       ...
       $html['_html'] = $this->load->view('public/user-dashboard/UserSingleBookingView',$data,true);
       return $this->output->set_content_type('application/json')->set_output(json_encode($html));
  
	}

public function user_notification(){
	$this->load->view('public/user-dashboard/UserNotificationView');
}

public function user_rating(){
	$data['get_rating_data']=$this->CommonMdl->getResultByCond('tbl_rating',array('user_id'=>$this->session->userdata['user_id']),'*',array('col_name'=>'rating_id','order'=>'desc'));
...
public function user_rating_edit(){
 ...
 // print_data($data['user_rating_edit']);
	$html['_html'] = $this->load->view('public/user-dashboard/UserRatingModelView',$data,true);
    return $this->output->set_content_type('application/json')->set_output(json_encode($html));
