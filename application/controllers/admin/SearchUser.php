   ...
   function search_listing(){
      ...
      if($type == "facility"  || $type == "academy"  || $type == "event" && isset($_REQUEST['fac_latitude']) && isset($_REQUEST['fac_longitude']) && $_REQUEST['user_id'])
	  {
		...
		foreach($data['fac_data'] as $facKey=>$facVal){
			$fac_id=$facVal->fac_id;
		
		$data['facility_sport_data']=$this->CommonMdl->getResultByCond('tbl_facility_sport',array('fac_id'=>$facVal->fac_id),'fac_id,sport_id,sport_court,sport_indor,sport_outdor',$order_by='');
     // echo $this->db->last_query();
					// die;
	 // print_r($data['facility_sport_data']);
	 // die;
			foreach($data['facility_sport_data'] as $sportKey=>$sportVal){
				
					$data['sport_data']=$this->CommonMdl->getResultByCond('tbl_sport_list',array('sport_id'=>$sportVal->sport_id),'sport_id,sport_name,sport_icon,sport_image',$order_by='');
					
					// echo "<pre>";
					// print_r($data['sport_data']);
				}
				$data['rating_data']=$this->SearchApiMdl->getResultbysearch($facVal->fac_id);
				...
				$data['fac_data'][$facKey]->sport_list=$data['sport_data'];
			...
			// print_r(json_encode($data['fac_data']));
	  }
	  else if($type == "event"){
		  print_r($_POST);
	  }
