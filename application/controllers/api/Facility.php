		...
		if(isset($_REQUEST['page']) && trim($_REQUEST['page'])!=''){
		   if(isset($_REQUEST['start_date']) || isset($_REQUEST['end_date']) || isset($_REQUEST['fac_id'])){
				   ...
				   foreach($get_event_list as $key=>$val){
					    $total_event_booking_count=$this->CommonMdl->fetchNumRows('tbl_booking_event_detail',array('event_id'=>$val->event_id),$cond1='');
					    $event_max=$val->event_max_capicity_per_day-$total_event_booking_count;
						
						$get_event_list[$key]->booked_slot_count="$total_event_booking_count";
						$get_event_list[$key]->available_slot_count="$event_max";
				   ...
				   if(count($upcoming_events)==0)
				{
				   ...
				   foreach($upcoming_events as $key=>$val){
					   $total_event_booking_count=$this->CommonMdl->fetchNumRows('tbl_booking_event_detail',array('event_id'=>$val->event_id),$cond1='');
						...
						$upcoming_events[$key]->booked_slot_count="$total_event_booking_count";
						$upcoming_events[$key]->available_slot_count="$event_max";
	...
	if(trim(@$_REQUEST['fac_id']))
	{
	    ...
	    $get_data= $this->CommonMdl->getResultByCond('tbl_event',array('fac_id'=>trim($_REQUEST['fac_id']),'event_approval'=>'approved'),'*',array('col_name'=>'event_id','order'=>'desc'));
		 ...
		 foreach($get_data as $key=>$val){
			...
			$get_data[$key]->booked_slot_count="$total_event_booking_count";
			$get_data[$key]->available_slot_count="$event_max";
...
$result_array=array();
  if(isset($_REQUEST['batch_slot_id']) && isset($_REQUEST['start_date'])){
          $batch_slot_id=$_REQUEST['batch_slot_id'];
          $start_date=$_REQUEST['start_date'];
		 $booking_slot_detail=$this->ApiMdl->order_user_detail($batch_slot_id,$start_date);
		 ...
		 foreach ($booking_slot_detail as $key => $value) {
		 	...
		 	$slot_detail_array = $this->ApiMdl->slot_detail_func($booking_order_id);
			$booking_slot_detail[$key]->folder_name="icon";
		     foreach($slot_detail_array as $slotKey=>$slotVal){
			     $booking_slot_detail[$key]->booking_slot_listing = $slot_detail_array;
			     $booking_slot_detail[$key]->booking_slot_listing[$slotKey]->folder_name="icon";
           ...
           if(isset($_REQUEST['fac_id']))
			{
				...
				$facility_sport=$this->ApiMdl->getResultByCondratings($get_facility_data->fac_id);
