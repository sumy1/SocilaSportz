				...
				<div class="main-content-inner">
					...
					<div class="page-content">
						...
						<div class="alert <?=$msg_class;?> alert-dismissible mb-2" id="msg_show" role="alert"><?=$msg;?> </div>
							...
							<?php endif;?>
							        <form action="<?=base_url('admin/page/contactlisting');?>" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                                      <div class="col-md-3">
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<input class="date-picker-to" id="end_date" name="end_date" 
											value="<?php if(isset($end_date)){echo $end_date;}?>" type="text" readonly="" data-date-format="dd-mm-yyyy">
											<span class="">
											<i class="fa fa-calendar bigger-110 calin icon_bhi"></i>
											</span>
											<span class="errcontactDate" class="error"style="color:red;"> </span>
										</div>
									</div>
									<!--<div class="col-md-3">
										<div class="form-group">
									...
									<input type="submit" name="Submit" value="Submit" class="btn btn-info ordersmall" id="order_submit">
                                        <a href="<?=base_url('admin/page/contactlisting');?>" class="btn btn-info clearbtn" id="clearbtn">
									...
									<div class="col-xs-12">
										...
										<div class="table-header">
											...
											<form method="post" action="<?=base_url('admin/page/exportdatacontact');?>" id="excel_export">
										...
										<div>
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													<?php 
													if(isset($contactdata) && !empty($contactdata)){ $i=0 ;foreach($contactdata as $result){ $i++;
													...
													<tr>
														...
														<td><?= $result->query_email; ?></td>
														<td><?= $result->query_contact; ?></td>
														<td><?= $result->query_title; ?></td>
														<td><?php echo substr($result->query_message,0,11); ?></td>
														...
														<!--<td><?= date('d-m-Y',strtotime($result->update_on)); ?></td>-->
...
$(document).ready(function(){		
		 ...
		 var to_date= $('#end_date').val();
				...
				if(form_date!=''  && to_date!=''){
					if((parseInt(endateArr[0]) >= parseInt(startateArr[0])) && (parseInt(endateArr[1]) >= parseInt(startateArr[1])) && (parseInt(endateArr[2]) >= parseInt(startateArr[2]))){
						...
						// alert('hello');
						 $('.errcontactDate').html('End date should be greater than start date');
						  return false;
					}
				}
	...
	contactdata.addEventListener('click',function(){
	  ...
	  $.ajax({
		  type:'post',
		  url:'<?=base_url('admin/page/changeuserstatus') ?>',
