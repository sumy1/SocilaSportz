				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Interested Sports List</li>
						</ul><!-- /.breadcrumb -->
					...
					<div>
									<?php echo form_open_multipart('admin/master/interestedsport',array('class'=>'form-horizontal')); ?>
									...
									<div class="col-md-3">
											...
											<input class="date-picker-from" id="start_date"  class="start_dates" name="start_date" value="<?php if(isset($start_date)){echo $start_date;}?>" type="text" readonly data-date-format="dd-mm-yyyy"  />
									...
									<input class="date-picker-to" id="end_date"  class="end_dates" name="end_date" value="<?php if(isset($end_date)){echo $end_date;}?>" type="text" readonly data-date-format="dd-mm-yyyy" />
									...
									<span class="errCouponTDatesss" class="error" style="color:red;"> </span>
									...
									<?php echo form_submit(['class'=>'btn btn-info ordersmall','name'=>'Submit','id'=>'interested_sport_submit','value'=>'Submit']); ?>   
                                    <a href="<?=base_url('admin/master/interestedsport');?>" class="btn btn-info clearbtn" id="clearbtn">
							...
							<?php endif;?>
									<div class="col-xs-12">
										...
										<div class="table-header">
											...
											<form method="post" action="<?=base_url('admin/master/exportdatabyinterestedsport');?>" id="excel_export">
										...
										<div>
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													...
													<?php 
													if(isset($user_sport_list) && !empty($user_sport_list)){ $i=0 ;foreach($user_sport_list as $sportKey=>$sportVal){ $i++;
													...
													<tr>
														...
														<td class="center"><?= $i ?></td>
		                                                   ...
		                                                   <td><?=$sportVal->user_name;?></td>
															<?php $sport_list=$this->AdminMdl->getInterestedsport($sportVal->user_id);
															$sport = '';
														 ...
														 <td><?php echo date('d-m-Y',strtotime($sportVal->created_on));?></td>
...
$(document).ready(function(){
	$(document).on('click','#interested_sport_submit',function(){
		 ...
		 var to_date= $("input[name='end_date']").val();
				...
				if(form_date!=''  && to_date!=''){
					if((parseInt(endateArrs[0]) >= parseInt(startateArrs[0])) && (parseInt(endateArrs[1]) >= parseInt(startateArrs[1])) && (parseInt(endateArrs[2]) >= parseInt(startateArrs[2]))){
						//alert('hello');
						$('.errCouponTDatesss').html('');
					}else{
						$('.errCouponTDatesss').html('End date should be greater than start date');
						  return false;
					}
				}
	});
...
var userinterestedsport_data=document.querySelector('#userinterestedsport_data');
	userinterestedsport_data.addEventListener('click',function(){
