...
<body id="searchfacilityacademy">
							...
							<div class="col-md-1 bmd-form-group is-filled samegap1" style="margin-top: -8px; margin-left: 15px;">
								...
								<div class="col-md-3 ">
									<div class="bmd-form-group" style="margin-top: -30px;">
										<input type="text" class="fog google_address" id="usercity" name="usercity" value="<?php if(!empty($this->session->userdata('usercity'))){ echo $this->session->userdata('usercity'); } ?>">
    ...
    <section class="search-wrapper">
        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-8">
                    <h3><span id="total_count"> <?php if(!empty($fac_data_count)) { echo $fac_data_count; } ?></span> results found</h3>
                </div>
                                ...
                                <div class="row">
                                    <div class="text-center col-sm-12">
										<form method="post" action="<?php echo base_url('booking'); ?>">
											...
											<input type="hidden" name="book_type" value="<?php if(isset($type)){echo $type;}else{ echo @$facVal->fac_type; } ?>" >
            ...
            if ($(window).scrollTop() + $(window).height() == $(document).height()) {
                ...
                if(total_page > page_count){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '<?=base_url();?>searchlisting/apply_filter',
                        data:{latitude:latitude,longitude:longitude,usercity:usercity,f_sport:f_sport,fac_type:fac_type,amenities_ids:amenities_ids,start_limit:start_limit},
                        success:function(res){ 
                            hiddingLoader();
                            $("#ajax_facilities_list").append(res['_html']);
                        }
                    });
                }
            } 
        });
$("span.heading").each(function(){var mgh = $(this).height(); if(mgh > 29){$("span.heading").height("50")}});
    </script>
