...
#dashboard_ajax_view #customerSearch .btn_proceed {
    margin-top: -21px !important;
}

#dashboard_ajax_view #customerSearch  .cus_modal_body{padding-bottom:2px !important;}

.date_data .fa-calendar{    color: #000 !important;}
				...
				<a href="<?=base_url('facility/booking');?>" title="">
					<div class="stat-container1 clearfix">
						...
						<div class="stat-count1">
							...
							<p class="stat-title">Confirmed Booking</p>								
						</div>
						<div class="clearfix"></div>
	...
	<div class="col-md-4 pd-left-default">
		...
		<div class="profile_complete_indicatior">
			...
			<div class="btn-conatiner text-center">
				<a href="<?=base_url('dashboard/accountinfo');?>" class="btn btn-sm btn-raised btn_proceed">Complete Profile</a>
			</div>
		</div>
		<?php } ?>
		
								<div class="cus_modal_header clearfix">
						<a class="toggle" ></a><h5 class="title"><a class="toggle"><i class="fa fa-calendar prefix"></i> Latest Events </a></h5>
					</div>
								...
								<div class="single-event">
									...
									<div class="payment">
										...
										<div class="event-payment2">
											<p>Booked slot</p>
											<span class="booking-number"><?php echo $total_event_booking_count=$this->CommonMdl->fetchNumRows('tbl_booking_event_detail',array('event_id'=>$events->event_id),$cond1='');?></span>
										</div>
										<div class="event-payment3">
											<p>Available slot</p>
											<span class="booking-number"><?=$events->event_max_capicity_per_day - $total_event_booking_count;?></span>
										</div>
									</div>
								</div>
							</div>
			...
			<div class="cus_modal">
									...
									<li class="media active">
										<a href="JavaScript:Void(0)" class="w-100">
											<div class="media-body">
												...
												<h5 class="title mt-0 mb-1"><?=$queries->user_name;?></h5>
												<h5 class="title mt-0 mb-1 user_email"><?=$queries->user_email;?></h5>
				...
				<div class="col-md-5 no-pad dashboardcalandarwrapper">
					...
					<div class="row text-center" style="position:relative">
						...
						<div class="text-center col-sm-12" >
                          <a href="<?=base_url('facility/booking');?>" class="btn btn-sm btn-raised btn_proceed mb-3">Go to Bookings</a>
				...
				<div class="col-md-3 pd-right-default">
					...
					<div class="customerSearch__m8place">
						<div class="cus_modal">
							<div class="cus_modal_header clearfix">
								<a class="toggle" ><h5 class="title"><i class="fa fa-search"></i>Book Now</a></h5>
							</div>
...
<img alt="sports icon" class="sportsimgicon redicon" src="<?=base_url('assets/public/images/archery.png');?>">
										<img alt="sports icon" class="sportsimgicon blueicon" src="<?=base_url('assets/public/images/sports_blue.png');?>">
														...
														<div class="form-group bmd-form-group search_mobile is-filled">
															...
															<span class="error" id="errbookingstartdate"></span>
														</div>
						...
						<div class="rating__m8place">
							<div class="cus_modal">
								...
								<a href="<?=base_url('facility/review');?>" class="btn btn-sm btn-raised btn_proceed">View Detail</a>
								</div>
							</div>								
						</div>
					...
					<div class="modal-dialog">
						...
						<div class="modal-content">
							<div class="modal-header">
								...
								<h5 class="modal-title pl-3" id="exampleModalLongTitle">Book Now<span class="sportname"></span></h5>
	...
	if(parseInt(startateArr[2]) >= parseInt(currentdateArr[2])){
		...
		if((parseInt(startateArr[1]) >= parseInt(currentdateArr[1])) ||(parseInt(startateArr[2]) > parseInt(currentdateArr[2]))){
			...
			if((parseInt(startateArr[0]) >= parseInt(currentdateArr[0])) || (parseInt(startateArr[1]) > parseInt(currentdateArr[1])) || (parseInt(startateArr[2]) > parseInt(currentdateArr[2]))){
				...
				$('#errbookingstartdate').show();
				$('#errbookingstartdate').html("You can not enter back date");
				$('html,body').animate({scrollTop: $("#errbookingstartdate").offset().top - 120},'slow');
				return false;
			}				
		}else{
			$('#errbookingstartdate').show();
...
<script>
	...
	function myFunction() {
		...
		$('.date_data').each(function(){
			...
			if(mmm < rrr && mmm > 0){$(this).addClass("fewavailable");}
...
/*	jQuery("#prevmonth").on("click", function(){ 
		...
		setTimeout(function(){ 
			...
			for(i; i <= count1; i++)
			{
				...
				var bbb = ndf.toString();
				var cdf = bbb.split(month[n])[0];
				jQuery("#dateRange").append('<div class="item"><a href="javascript:void(0)"><div class="date_data"><input type="radio" name="rGroup" value="1" /><span class="week">'+ cdf +'</span><span class="day">'+ i +'</span> <span class="availableseat">0/0</span>          <i class="fa fa-calendar"></i><i class="fa fa-check-circle" aria-hidden="true"></i></div></a></div>');
				jQuery("#monthname").html(month[n]);
				jQuery("#yearname").html(abc);
			...
			if(hhh == "Jan")
			{
				...
				for(i; i <= count1; i++)
				{
					...
					var bbb = ndf.toString();
					var cdf = bbb.split("Dec")[0];
					jQuery("#dateRange").append('<div class="item"><a href="javascript:void(0)"><div class="date_data"><input type="radio" name="rGroup" value="1" /><span class="week">'+ cdf +'</span><span class="day">'+ i +'</span> <span class="availableseat">0/0</span>          <i class="fa fa-check-circle" aria-hidden="true"></i></div></a></div>');
