...
<span id="errslot" class="error"> </span>
	<div class="col-sm-8">
			<h5 class="modal-title" id="exampleModalLongTitle" style="font-size:17px; margin-bottom:10px;"><?=$fac_name;?> </h5>
	</div>
		<div class="col-sm-4 text-right" style="font-size:17px; margin-bottom:10px;"><?=date('d-M-Y', strtotime($date))?></div>

		

			<?php if(isset($fac_slots) && !empty($fac_slots)){
				foreach ($fac_slots as $slot) {
				 ...
				 $count=$this->CommonMdl->fetchNumRows('tbl_booking_slot_detail',array('batch_slot_id' => $slot->batch_slot_id,'start_date'=>date('Y-m-d',strtotime($date))),$cond1='');

				 ?>

				<label class="btn <?php if($count>0) echo "booked";?>" data-original-title="" title="">
				<input type="checkbox" name="options[]" class="optionslot batch_slot_id" autocomplete="off" value="<?=$slot->batch_slot_id;?>"> 
				<div class="plan">
						...
						<?php } ?>
							...
							<div class="row">
								<div class="col-sm-12 text-right" style="padding-top:20px;">
									<button type= "submit" class="bookedslot orange-btn">Book Now</button><br>
