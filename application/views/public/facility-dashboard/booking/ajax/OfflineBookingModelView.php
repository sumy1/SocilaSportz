...
<?php if (isset($fac_type) && $fac_type=='facility') { ?>
	...
	if(isset($fac_slots) && !empty($fac_slots)){
		...
		<?php 
			...
			foreach ($fac_slots[$k] as $slot) {
				 $cat_name=$this->CommonMdl->getRow('tbl_batch_slot_type','batch_slot_type',array('batch_slot_type_id' => $slot->batch_slot_type_id));
				  $slot_price=$this->CommonMdl->getRow('tbl_slot_package_price','slot_package_price',array('batch_slot_id' => $slot->batch_slot_id));
				...
				<label class="available_batch_slot btn <?php if($count>0) echo "booked";?>" data-original-title="" title="">
					...
					<div class="plan">
						...
						<span class="amount" >  <span class="time"><i class="fa fa-inr"></i> <?=$slot_price->slot_package_price;?></span><br>
		...
		<?php }else{ ?>
				...
				<div class="col-sm-9 nodata">No data Available </div>
				<div class="col-sm-3 text-right" style="font-size:17px; margin-bottom:10px;"><?=date('d-M-Y', strtotime($show_date[$k]))?></div>
		<?php } echo "</br><hr>"; } ?>		
...
<?php if (isset($fac_type) && $fac_type=='academy') { ?>
			...
			<?php if(isset($fac_batch) && !empty($fac_batch)){
				foreach ($fac_batch as $batch) {
					 ...
					 $count=$this->CommonMdl->fetchNumRows('tbl_booking_slot_detail',array('batch_slot_id' => $batch->batch_slot_id,'start_date'=>date('Y-m-d',strtotime($date))),$cond1='');?>
							...
							<label class="available_batch_slot btn <?php if($count==$batch->max_participanets){ echo "booked";} else {echo "green";}?>" data-original-title="" title="">
			...
			<?php } ?>
						...
						<?php } ?>
							...
							<div class="row">
								<div class="col-sm-6">
								<span id="errslot" style="bottom: 40px;" class="error"> </span>
							</div>
...
<script>
	...
	var abc = $(".available_batch_slot").length;
		 if(abc < 1){
	 	$(".bookedslot.orange-btn").hide();
	 	$('.nodata').remove();
	 	$('#model_ajax_div').append('<div class="col-sm-12 nodata">No data Available</div>');
	};
